...
class TestConfluenceV2Adapter:
    ...
    @pytest.mark.parametrize(
        ...
        [("append", "after"), ("before", "before"), ("after", "after")],
    )
    def test_root_move_position(
        self, v2_adapter, mock_session, position, rest_position
    ):
        space_response = Mock()
        space_response.json.return_value = {"results": [{"id": "900"}]}
        roots_response = Mock()
        roots_response.json.return_value = {"results": [{"id": "901"}]}
        mock_session.get.side_effect = [space_response, roots_response]

        v2_adapter.move_page("111", position=position, target_space_key="DEST")

        mock_session.put.assert_called_once_with(
            f"https://example.atlassian.net/wiki/rest/api/content/111"
            f"/move/{rest_position}/901"
        )
