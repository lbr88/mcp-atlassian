...
class TestPagesMixin:
    ...
    def test_cloud_child_metadata_does_not_require_body_expansion(self, pages_mixin):
        """A metadata-only child request retains the page's version and dates."""
        pages_mixin.config = MagicMock(url="https://example.atlassian.net/wiki")
        pages_mixin.config.is_cloud = True
        adapter = MagicMock()
        adapter.get_page_direct_children.return_value = {
            "results": [{"id": "789012", "title": "Child", "type": "page"}]
        }
        adapter.get_page.return_value = {
            "id": "789012",
            "title": "Child",
            "type": "page",
            "version": {
            ...
            "history": {
                ...
                "lastUpdated": {"when": "2026-01-02T12:00:00Z"},
            },
        }
        ...
        with patch.object(
            ...
            prop.return_value = adapter
            children = pages_mixin.get_page_children("123", expand="version,history")

        assert children[0].version.number == 7
        assert children[0].version.by.display_name == "Editor"
        assert children[0].created == "2026-01-01T12:00:00Z"
        assert children[0].updated == "2026-01-02T12:00:00Z"
...
class TestUpdatePageSection:
    ...
    def test_section_heading_inside_code_macro_is_not_modified(self, pages_mixin):
        """Identical heading markup in CDATA never becomes the insertion point."""
        macro = (
        ...
        raw_page = self._make_page(
            "1", "P", f"{macro}<h2>Target</h2><p>old</p><h2>Other</h2><p>keep</p>"
        )

        def save_page(**kwargs):
            return self._make_page("1", "P", kwargs["body"])

        with (
            patch.object(pages_mixin, "get_page_content", return_value=raw_page),
            patch.object(pages_mixin, "update_page", side_effect=save_page),
        ):
            result = pages_mixin.update_page_section(
                "1", "Target", "<p>new</p>", content_format="storage"
            )

        assert result.content == (
            f"{macro}<h2>Target</h2><p>new</p><h2>Other</h2><p>keep</p>"
        )
