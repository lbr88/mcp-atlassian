...
class TestPermissionsMixin:
    ...
    def permissions_mixin(self):
        ...
        mixin._v1_rest_base_url = lambda: PermissionsMixin._v1_rest_base_url(mixin)
    ...
    def test_content_permissions_use_oauth_v1_gateway(self, permissions_mixin):
        """OAuth permission checks use the Cloud v1 gateway's wiki context."""
        permissions_mixin.config.auth_type = "oauth"
        permissions_mixin.confluence.url = (
            "https://api.atlassian.com/ex/confluence/cloud-id"
        )
        response = MagicMock()
        response.json.return_value = {"hasPermission": True}

        def post(url, **kwargs):
            assert url == (
                "https://api.atlassian.com/ex/confluence/cloud-id"
                "/wiki/rest/api/content/123/permission/check"
            )
            return response

        permissions_mixin.confluence._session.post.side_effect = post
        assert permissions_mixin.check_content_permissions("123", "user", "read") == {
            "hasPermission": True
        }
