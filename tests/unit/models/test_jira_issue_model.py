...
@pytest.mark.parametrize(
    ("markdown", "expected"),
    [
        ("Blocked by PROJ-123 and PROJ-456.", "Blocked by PROJ-123 and PROJ-456."),
        ("- [x] Deploy\n- [ ] Verify", "[x] Deploy\n[ ] Verify"),
        (
...
def test_description_adf_readback_preserves_visible_content(markdown, expected):
    """Cloud descriptions retain inline text, task states, and expand headings."""
    ...
    issue = JiraIssue.from_api_response(
        {
            ...
            "fields": {
                ...
                "description": markdown_to_adf(
                    markdown, jira_base_url="https://example.atlassian.net"
                ),
            },
        }
    )

    assert issue.to_simplified_dict()["description"] == expected
