...
async def test_delete_attachment_blocked_in_read_only_mode(
    ...
    with patch(
        ...
        with pytest.raises(ToolError, match="read-only mode"):
            await delete_attachment_tool(ctx, "10001")
...
@pytest.mark.anyio
async def test_add_comment_explicit_internal_visibility_is_preserved(
    jira_client, mock_jira_fetcher
):
    """An internal JSM comment stays private without an operator project guard."""
    from mcp_atlassian.jira.comments import CommentsMixin
    from mcp_atlassian.jira.config import JiraConfig as ClientJiraConfig

    config = ClientJiraConfig(
        url="https://jira.example.test",
        auth_type="pat",
        personal_token="test-token",
        internal_only_projects=frozenset(),
    )
    api = MagicMock()
    api.default_headers = {}
    ...
    with patch("mcp_atlassian.jira.client.Jira", return_value=api):
        comments = CommentsMixin(config=config)
    mock_jira_fetcher.add_comment.side_effect = comments.add_comment
    mock_jira_fetcher._is_internal_only_project.side_effect = (
        comments._is_internal_only_project
    )

    response = await jira_client.call_tool(
        "jira_add_comment",
        {"issue_key": "TEST-123", "body": "Internal note", "public": False},
    )
