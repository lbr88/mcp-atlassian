...
class TestFixAttachmentImages:
    ...
    def test_relative_path_references_uploaded_basename(self):
        html = '<img src="images/chart%201.png" alt="Chart"/>'
        result = self.fix(html)
        assert 'ri:filename="chart 1.png"' in result

    def test_xml_sensitive_values_are_escaped(self):
        html = '<img src="chart & q.png" alt="A & B"/>'
        result = self.fix(html)
        assert 'ac:alt="A &amp; B"' in result
        assert 'ri:filename="chart &amp; q.png"' in result
