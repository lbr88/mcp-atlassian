...
class TestIssuesMixin:
    ...
    def test_update_issue_textarea_and_status_uses_cloud_v3(
        self, issues_mixin: IssuesMixin, make_issue_data, status_first
    ):
        """A status change cannot send ADF to v2 or discard subsequent fields."""
        issues_mixin.get_field_by_id = MagicMock(
            return_value={
                "name": "Resolution Summary",
                "schema": {
        ...
        issues_mixin.get_available_transitions = MagicMock(
            return_value=[{"id": "21", "name": "Done", "to_status": "Done"}]
        )
        issues_mixin.jira.get_issue.return_value = make_issue_data(status="Done")
        issues_mixin.jira.update_issue.side_effect = ValueError("ADF requires v3")
        writes = []

        def put_api3(path, payload):
            document = payload["fields"]["customfield_10078"]
            assert document["type"] == "doc"
            writes.append(path)
            return {}

        issues_mixin._put_api3 = MagicMock(side_effect=put_api3)
        fields = {"status": "Done", "customfield_10078": "## Summary"}
        if not status_first:
            fields = dict(reversed(list(fields.items())))

        result = issues_mixin.update_issue("TEST-123", **fields)

        assert result.status.name == "Done"
        assert writes == ["issue/TEST-123"]
