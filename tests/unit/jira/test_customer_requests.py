...
def test_create_customer_request_preserves_cascading_child(
    jira_fetcher: JiraFetcher,
) -> None:
    """A canonical cascading option retains the child's selection in the request."""
    fetcher = _make_customer_requests_fetcher(jira_fetcher)
    fetcher.jira.get = MagicMock(
        return_value={
            "requestTypeFields": [
                {
                    ...
                    "jiraSchema": {
                        ...
                        "custom": (
                            ...
                            "customfieldtypes:cascadingselect"
                        ),
                    },
                    "validValues": [{"value": "10112", "label": "Parent"}],
                }
            ]
        }
    )
    selection = {"id": "10112", "child": {"id": "10115"}}

    def create_request(url, *, data, **kwargs):
        assert data["requestFieldValues"]["customfield_10001"] == selection
        return {"issueId": "10010", "issueKey": "SUP-101"}

    fetcher.jira.post = MagicMock(side_effect=create_request)
    result = fetcher.create_customer_request(
        service_desk_id="4",
        request_type_id="23",
        request_field_values={"customfield_10001": selection},
    )
