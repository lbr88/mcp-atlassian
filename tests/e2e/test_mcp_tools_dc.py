...
class TestMCPJiraTools:
    ...
    async def test_jira_assign_issue(
        ...
        created = await call_tool(
            ...
            {
                "project_key": dc_instance.project_key,
                "summary": f"DC MCP Assign Test {uuid.uuid4().hex[:8]}",
                "issue_type": "Task",
            },
        )
        assert not created.is_error
        issue_key = json.loads(created.content[0].text)["issue"]["key"]
        try:
            assign_result = await call_tool(
                mcp_client,
                "jira_assign_issue",
                {"issue_key": issue_key, "assignee": dc_instance.admin_username},
            )
            ...
            await call_tool(mcp_client, "jira_delete_issue", {"issue_key": issue_key})
