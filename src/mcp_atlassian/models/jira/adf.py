...
def adf_to_text(adf_content: dict | list | str | None) -> str | None:
    ...
    if isinstance(adf_content, dict):
        ...
        if node_type in ("paragraph", "heading", "taskItem", "codeBlock"):
            text = "".join(
                adf_to_text(child) or "" for child in adf_content.get("content", [])
            )
            if node_type == "taskItem":
                completed = adf_content.get("attrs", {}).get("state") == "DONE"
                return f"[{'x' if completed else ' '}] {text}"
            if node_type == "codeBlock":
                return f"```\n{text}\n```"
            return text or None

        if node_type in ("expand", "nestedExpand"):
            title = adf_content.get("attrs", {}).get("title", "")
            body = adf_to_text(adf_content.get("content")) or ""
