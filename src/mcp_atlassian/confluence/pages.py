...
class PagesMixin(ConfluenceClient):
    ...
    def _get_v2_page_children_items(
        ...
        *,
        ...
        if any(section in expand for section in ("body", "version", "history")):
            return self._enrich_v2_child_pages(v2_adapter, child_items, expand)

        return child_items

    def _enrich_v2_child_pages(
        self,
        v2_adapter: ConfluenceV2Adapter,
        child_items: list[dict[str, Any]],
        expand: str,
    ) -> list[dict[str, Any]]:
        """Fetch requested content and metadata for v2 direct-child pages."""
    ...
    def update_page_section(
        ...
        *,
        ...
        # Anchor insertion to the selected node, not matching serialized markup
        # that can also occur in another section or a code macro's CDATA.
        for node in siblings_to_remove:
            node.extract()
        fragment = BeautifulSoup(new_storage_fragment, "html.parser")
        for node in reversed(list(fragment.contents)):
            target_heading.insert_after(node.extract())
        final_html = str(soup)
